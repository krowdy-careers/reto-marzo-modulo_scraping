...
export const getItemsTottus = async (baseUrl: string): Promise<any[]> => {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        cluster.execute(url, async ({ page, data: url }) => {
            ...
            for (let i = 0; i < 8; i++) { // asegurar que se obtiene url de las imagenes
            ...
            const categoria = urlParts[urlParts.length - 1] || "Categoría desconocida";

            const products = await page.evaluate((categoria) => {
